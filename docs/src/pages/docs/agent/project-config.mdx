...
# max_retries = 10        # resend attempts after a failed upstream call; 0 disables retrying
# retry_backoff_ms = 250  # base delay before the first retry, doubled per attempt and jittered
...
| `max_retries` | `10` | Attempts to resend a failed upstream call, counted *after* the first failure. Covers a dropped or refused connection, a request timeout, HTTP 429/502/503/504, and a completion that came back with no answer. Set `0` to disable retrying - a real setting, distinct from leaving the key unset |
| `retry_backoff_ms` | `250` | Base delay before the first retry. Doubles per attempt, capped at 30s, then jittered down into `[0.5x, 1.0x]` so many clients do not retry one provider in lockstep. A `Retry-After` header from the upstream replaces the computed delay (honoured up to 60s) |
